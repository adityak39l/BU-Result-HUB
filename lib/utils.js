...
export function searchStudents(query = '', selectedBranches = 'ALL', selectedYear = 'ALL') {
  ...
  // 2. Filter by Passing / Batch Year (2024-25, 2025-26, 2023-24, etc.)
  if (selectedYear !== 'ALL') {
    list = list.filter(s => {
      const yr = selectedYear.toLowerCase();
      if (yr.includes('2024') || yr === '2024-25') {
        return (s.batch && s.batch.includes('2024')) || (s.semesters && s.semesters.some(sem => sem.sem === 3 || sem.sem === 4));
      }
      if (yr.includes('2025') || yr === '2025-26') {
        return (s.batch && s.batch.includes('2025')) || (s.semesters && s.semesters.some(sem => sem.sem === 5 || sem.sem === 6));
      }
      if (yr.includes('2023') || yr === '2023-24') {
        return (s.batch && s.batch.includes('2023')) || (s.semesters && s.semesters.some(sem => sem.sem === 1 || sem.sem === 2));
      }
