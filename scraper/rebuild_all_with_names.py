...
def extract_cse_student(roll_str):
    ...
    if not s_name or not semesters:
        return None
...
for chunk in student_chunks:
    try:
        st = json.loads(chunk)
        if st.get("branch") != "CSE":
            existing_non_cse.append(st)
    except:
        pass

print(f"Preserved {len(existing_non_cse)} existing non-CSE students (EIE, BME, ME).")

# Extract all valid CSE students
cse_students = []
for r in range(231381030001, 231381030091):
    roll_str = str(r)
    cse_st = extract_cse_student(roll_str)
    if cse_st:
        cse_students.append(cse_st)

print(f"\nExtracted {len(cse_students)} valid CSE students:")
for st in cse_students:
    print(f"  {st['rollNo']} -> {st['name']} (CGPA: {st['cgpa']}, Sems: {[s['sem'] for s in st['semesters']]})")

all_students = existing_non_cse + cse_students
print(f"\nTotal combined students in database: {len(all_students)}")
...
print("\nlib/data.js cleanly generated with 100% verified student names!")
