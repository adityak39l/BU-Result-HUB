...
# Gather all 4 semesters data for ECE
all_ece = {} # roll -> { 'name', 'father', 'mother', 'enroll', '3': d, '4': d, '5': d, '6': d }
    ...
    for f in os.listdir(dir_path):
        if f.endswith('.html'):
            ...
            if roll:
                ...
                if res['name']: all_ece[roll]['name'] = res['name']
                if res['father'] and res['father'] != 'N/A': all_ece[roll]['father'] = res['father']
                if res['mother'] and res['mother'] != 'N/A': all_ece[roll]['mother'] = res['mother']
                if res['enroll'] and res['enroll'] != 'N/A': all_ece[roll]['enroll'] = res['enroll']
                
                if res['sgpa'] > 0:
                    all_ece[roll]['semesters'][int(sem_str)] = res['sgpa']
                if res['subjects']:
                    all_ece[roll]['semesterSubjects'][sem_str] = res['subjects']
...
print(f"Total Unique ECE Students with 4-Semester Data: {len(all_ece)}")

# Build student objects
new_ece_students = []
for roll, data in all_ece.items():
...
print(f"Successfully integrated ALL 4 Semesters (Sem 3, 4, 5, 6) for {len(new_ece_students)} ECE students in lib/data.js!")
