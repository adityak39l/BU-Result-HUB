...
def parse_html_marksheet(file_path, sem_num):
    ...
    for tr in soup.find_all('tr'):
        ...
        if len(tds) >= 11:
            ...
            # Extract Max Marks from header definitions
            # Theory
            if is_theory:
                ext_val = tds[2]
                int_val = tds[4]
                ...
                # Check Int Max from tds[3]
                if len(tds) > 3 and tds[3]:
                    if '30' in tds[3]: int_max = 30
                    elif '25' in tds[3]: int_max = 25
                    elif '50' in tds[3]: int_max = 50
                    else: int_max = 25
                else: int_max = 25
                
                # Check Ext Max from tds[7]
                if len(tds) > 7 and tds[7]:
                    if '70' in tds[7]: ext_max = 70
                    elif '50' in tds[7]: ext_max = 50
                    elif '25' in tds[7]: ext_max = 25
                    else: ext_max = 50
                else: ext_max = 50
            ...
            if is_theory:
                ...
                if official_grade in ['O', 'A+', 'A', 'B+', 'B', 'C', 'F']:
                    ...
                    pct = (tot_num / 50.0 * 100) if tot_num > 0 else 0
                    if pct >= 90: grade = "O"
                    elif pct >= 80: grade = "A+"
                    elif pct >= 70: grade = "A"
                    elif pct >= 60: grade = "B+"
...
for roll in sorted(list(all_cse_rolls)):
    ...
    p3 = parse_html_marksheet(f3, 3)
    p4 = parse_html_marksheet(f4, 4)
    p5 = parse_html_marksheet(f5, 5)
    p6 = parse_html_marksheet(f6, 6)
    ...
    # Payal Jha & Reena Yadav Sem 5/6 fallback if not on portal
...
print("lib/data.js written with exact lab max marks and 4-semester data!")
